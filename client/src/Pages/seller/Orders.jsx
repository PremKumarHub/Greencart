                    ...
                    <div key={index} className="flex flex-col md:items-center md:flex-row gap-5 justify-between p-5 max-w-4xl rounded-md border border-gray-300">
                                ...
                                {order.items.map((item, i) => (
                                    <div key={i} className="flex flex-col">
                                        <p className="font-medium">
                                            {item.productId?.name || item.product?.name || 'Product'}{" "} <span className="text-primary">x {item.quantity}</span>
                                        </p>
                                    </div>
                                ))}
                            </div>
                        ...
                        <div className="text-sm md:text-base text-black/60">
                            {order.address && typeof order.address === 'object' ? (
                                ...
                                <p className='text-black/80'>{order.address || 'Address not available'}</p>
                            )}
                        </div>
